...
case $COMMAND in
    ...
    host)
        ...
        NODE_ROLE=host ./bin/boot$BIN_EXT $HOST_PORT > logs/host.log 2>&1 &
    ...
    client)
        ...
        NODE_ROLE=client_manual ./bin/boot$BIN_EXT "$2" "$3" > "logs/client_$2.log" 2>&1 &
        ;;
    ...
    attach)
        ...
        for ((i=1; i<=BOT_COUNT; i++)); do
            ...
            NODE_ROLE=bot_$i ./bin/boot_headless$BIN_EXT $CLIENT_PORT $LOBBY_ID > "logs/client_${CLIENT_PORT}.log" 2>&1 &
    ...
    swarm)
        ...
        for ((i=1; i<=GRAPHICAL_CLIENTS; i++)); do
            ...
            NODE_ROLE=client_$CLIENT_IDX ./bin/boot$BIN_EXT $CLIENT_PORT $LOBBY_ID > "logs/client_${CLIENT_PORT}.log" 2>&1 &
            SWARM_PIDS+=($!)
        ...
        for ((i=1; i<=BOT_CLIENTS; i++)); do
            CLIENT_PORT=$((HOST_PORT + CLIENT_IDX))
            NODE_ROLE=bot_$CLIENT_IDX ./bin/boot_headless$BIN_EXT $CLIENT_PORT $LOBBY_ID > "logs/client_${CLIENT_PORT}.log" 2>&1 &
            SWARM_PIDS+=($!)
