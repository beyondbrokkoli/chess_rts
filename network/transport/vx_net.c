...
EXPORT int vx_net_recv_all(RxPacket* out_buffer, int max_count) {
    ...
    while (count < max_count) {
        ssize_t recvd = recvfrom(g_net.sock,
                                 ...
                                 CFG_MAX_PACKET_SIZE, // Use the constant
                                 0,
        ...
        // THE MACHETE DISCRIMINATOR
        if (recvd == sizeof(IcePunchPacket)) {
            IcePunchPacket* punch = (IcePunchPacket*)out_buffer[count].data;
            token = punch->session_token;
            pid = punch->player_id;
        } 
        // Ensure it contains at least the Lockstep header (offsetof evaluates dynamically at compile time)
    ...
    for (int wait = 0; wait < 50; wait++) {
        ...
        if (recvd >= 20) {
            ...
            while (offset < 20 + msg_len && offset + 4 <= recvd) {
                ...
                if (attr_type == 0x0020) {
                    // HARDENED STUN: Enforce 8-byte length and prevent out-of-bounds reads
                    if (attr_len >= 8 && (offset + 12) <= recvd) {
                        uint8_t family = resp[offset+5]; // Fix Qwen's off-by-one: +4 is reserved 0x00, +5 is Family
                        if (family == 0x01) { // IPv4
