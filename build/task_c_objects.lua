return function(ctx)
    ...
    if ctx.platform == "linux" then
        ...
        ctx.run_cmd("gcc network/vx_net.c -O3 -march=x86-64-v3 -shared -fPIC -o bin/libvx_net.so")

        -- 2. Build the Top-Down Host Unity Build
        print(" |- Compiling host engine (boot.elf)...")
        local linux_build = "gcc host/main.c -O3 -march=x86-64-v3 -Wl,-E -I/usr/include/luajit-2.1 -lglfw -lvulkan -lluajit-5.1 -lm -lpthread -o bin/boot.elf"
    ...
    elseif ctx.platform == "win" then
        ...
        ctx.run_cmd(string.format('gcc network/vx_net.c -O3 -march=x86-64-v3 -shared -lws2_32 -o bin/vx_net.dll'))
        ...
        local win_build = string.format(
            'gcc host/main.c -O3 -march=x86-64-v3 -Wl,--export-all-symbols,--no-insert-timestamp -I"%s" -I"%s/Include" -L"%s/Lib" -lglfw3 -lvulkan-1 -lluajit-5.1 -lm -o bin/boot.exe',
